on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/com.kdesk.desktop_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/com.kdesk.desktop_service.plist"
  set app_bundle to "/Applications/KDesk.app"
  ...
  set kill_others to "pids=$(pgrep -x 'KDesk' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
